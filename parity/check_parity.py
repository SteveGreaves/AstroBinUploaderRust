#!/usr/bin/env python3
"""
Differential check for the Rust port's emulated parsers.
...
libraries they emulate -- a drift the byte-level output diff would not catch
until much later phases, if at all.

The fixtures, references and golden_config.ini in this directory are copies
taken from the Python project (SteveGreaves/AstroBinUploader,
golden_tests/) so that this repository tests standalone. Re-copy them
whenever the Python side re-blesses its references; parity/CORPUS.md records
the commit they came from.

Requires configobj and pandas -- the libraries being emulated:
    pip install configobj pandas
...
GOLDEN_CONFIG = PARITY_DIR / "golden_config.ini"
FIXTURES = sorted((PARITY_DIR / "fixtures").glob("*_raw.csv"))
RUST_BIN = REPO_ROOT / "target" / "debug" / "astrobin-upload"
...
def main() -> int:
    if not RUST_BIN.exists():
        raise SystemExit("build the binary first: cargo build")
