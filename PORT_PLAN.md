...
| **4** | FITS and XISF readers | The only part the CSV fixtures cannot exercise. ~~**Prerequisite:** hand-built FITS/XISF fixtures.~~ **Done 2026-09-08**: `parity/fixtures/binary/` holds 241 header-only files in three scenarios, with references blessed from a live v2.1.1 disk scan — see `parity/CORPUS.md`. Note the correction recorded there: a file named `.fits.fz` is *skipped* by the traversal filter, so the case A7 was written for is a compressed image inside a `.fits` file. Both are pinned. |
