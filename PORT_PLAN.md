...
| **4** | ~~FITS and XISF readers~~ — **done 2026-09-08**. `src/fits.rs` (hand-written, no cfitsio), `src/xisf.rs`, `src/extractor.rs`, `src/pathutil.rs`. `parity/check_readers.py`: 3/3 scenarios, every pipeline step cell-identical and both artifacts matching — including a scan of 221 real FITS reproducing the committed `sadr` reference. | The only part the CSV fixtures cannot exercise. Corpus built first (`parity/CORPUS.md`), including the correction that a file named `.fits.fz` is *skipped* by the traversal filter, so A7's case is a compressed image inside a `.fits` file. |
| **5** | ~~`rayon` parallelism; release matrix for Windows / Linux (`musl` static) / macOS, x86-64 and arm64~~ — **done 2026-09-08**. Row order preserved by construction (`par_iter().map().collect()`); measured 2.3x on this project's own rotational disk. `.github/workflows/release-matrix.yml` actually run three times on GitHub's real runners, fixing a Windows long-path checkout failure and a host-dependent `abspath` test each run found; all five legs green, five binaries built. | Optimise only once correct |
| **6** | ~~Differential harness in CI: both binaries over the full corpus, byte-compare modulo the `Generated` line~~ — **done 2026-09-08**. `.github/workflows/differential-harness.yml`, all four `check_*.py` on every push, report-only. Run for real on GitHub's runners, not just authored: all sixteen fixture/scenario checks confirmed passing, 1m30s. | Ongoing guarantee |
